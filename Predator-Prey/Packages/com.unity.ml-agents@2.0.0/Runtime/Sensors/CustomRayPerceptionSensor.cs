...
namespace Unity.MLAgents.Sensors
{
    ...
    public class CustomRayPerceptionSensor : ISensor //, IBuiltInSensor
    {
        ...
        CustomRayPerceptionInput m_CustomRayPerceptionInput;
        CustomRayPerceptionOutput m_CustomRayPerceptionOutput;
        ...
        /// <summary>
        /// Creates the CustomRayPerceptionSensor.
        /// </summary>
        ...
        {
            m_Name = name;
            m_CustomRayPerceptionInput = rayInput;

            SetNumObservations(rayInput.OutputSize());

            m_DebugLastFrameCount = Time.frameCount;
            m_CustomRayPerceptionOutput = new CustomRayPerceptionOutput();
        }

        /// <summary>
        /// The most recent raycast results.
        /// </summary>
        public CustomRayPerceptionOutput CustomRayPerceptionOutput
        {
            get { return m_CustomRayPerceptionOutput; }
        }
        ...
        internal void SetCustomRayPerceptionInput(CustomRayPerceptionInput rayInput)
        {
            ...
            if (m_CustomRayPerceptionInput.OutputSize() != rayInput.OutputSize())
            {
                Debug.Log(
        ...
        {
            using (TimerStack.Instance.Scoped("CustomRayPerceptionSensor.Perceive"))
            {
                Array.Clear(m_Observations, 0, m_Observations.Length);
                var numRays = m_CustomRayPerceptionInput.Angles.Count;
                var numDetectableTags = m_CustomRayPerceptionInput.DetectableTags.Count;

                // For each ray, write the information to the observation buffer
                for (var rayIndex = 0; rayIndex < numRays; rayIndex++)
                {
                    m_CustomRayPerceptionOutput.RayOutputs?[rayIndex].ToFloatArray(numDetectableTags, rayIndex, m_Observations);
                }
        ...
        public void Update()
        {
            ...
            if (m_CustomRayPerceptionOutput.RayOutputs == null || m_CustomRayPerceptionOutput.RayOutputs.Length != numRays)
            {
                m_CustomRayPerceptionOutput.RayOutputs = new CustomRayPerceptionOutput.RayOutput[numRays];
            }

            // For each ray, do the casting and save the results.
            for (var rayIndex = 0; rayIndex < numRays; rayIndex++)
            {
                m_CustomRayPerceptionOutput.RayOutputs[rayIndex] = PerceiveSingleRay(m_CustomRayPerceptionInput, rayIndex);
            }
        }
        ...
        public static CustomRayPerceptionOutput Perceive(CustomRayPerceptionInput input)
        {
            CustomRayPerceptionOutput output = new CustomRayPerceptionOutput();
            output.RayOutputs = new CustomRayPerceptionOutput.RayOutput[input.Angles.Count];
        ...
        /// <summary>
        /// Evaluate the raycast results of a single ray from the CustomRayPerceptionInput.
        /// </summary>
        /// <param name="input"></param>
        /// <param name="rayIndex"></param>
        /// <returns></returns>
        internal static CustomRayPerceptionOutput.RayOutput PerceiveSingleRay(
        ...
        )
        {
            ...
            var rayOutput = new CustomRayPerceptionOutput.RayOutput
            {
                HasHit = castHit,
